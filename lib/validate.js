...
module.exports = function() {
	...
	return function * (next) {
		...
		this.checkBody = function(key) {
			...
			return new Validator(this, key, body[key], key in body, body, keyPath );
		};
...
function Validator(context, key, value, exists, params, keyPath) {
...
Validator.prototype.addError = function(tip) {
	...
	var e = {};
    var key = this.keyPath ? this.keyPath + '.' + this.key : this.key;
	e[key] = tip;
...
Validator.prototype.decodeBase64 = function(inBuffer) {
	if (!this.hasError()&&this.value) {
		if(inBuffer){
	...
	if (this.goOn && this.value ) {
		...
		if('function' == typeof afterMove){
			if(isGeneratorFunction(afterMove)){
				yield afterMove(this.value.newPath,this.context,this.key);
			}else{
				afterMove(this.value.newPath,this.context,this.key);
			}
		}
	}
	return this;
	...
	if (this.goOn && this.value ) {
		...
		if('function' == typeof afterCopy){
			if(isGeneratorFunction(afterCopy)){
				yield afterCopy(this.value.newPath,this.context,this.key);
			}else{
				afterCopy(this.value.newPath,this.context,this.key);
			}
		}
	}
	return this;
