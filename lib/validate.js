...
module.exports = function(opt) {
	...
	return function * (next) {
		...
		// };
		this.checkParams = function(key) {
			return new Validator(this, key, this.request.params[key], key in this.request.params,this.request.params);
		};
		yield next;
	};
...
Validator.prototype.trim = function() {
	if (!this.hasError()) {
		this.params[this.key] = v.trim(this.value);
	}
};
Validator.prototype.ltrim = function() {
	if (!this.hasError()) {
		this.params[this.key] = v.ltrim(this.value);
	}
};
Validator.prototype.rtrim = function() {
	if (!this.hasError()) {
		this.params[this.key] = v.rtrim(this.value);
	}
};
...
Validator.prototype.stripLow = function(l) {
	if (!this.hasError()) {
		this.params[this.key] = v.stripLow(this.value, l);
	}
};
Validator.prototype.whitelist = function(s) {
	if (!this.hasError()) {
		this.params[this.key] = v.whitelist(this.value);
	}
};
Validator.prototype.blacklist = function(s) {
	if (!this.hasError()) {
		this.params[this.key] = v.blacklist(this.value);
	}
};
